...
2. To reproduce the results on our collected dataset:
   - Run the evaluation through `python a_final_code-lceclassifier_seq_strc.py --seq_model_name self_seq --feature_type seq_strc --reduced_dim_strc 28 --reduced_dim_seq 255 --result_dir_suffix _change_strc_embed`. Expected runing time: within 1 min Nvidia A100.
3. To reproduce the results on the third party dataset:
   - Run the evaluation through `python a_final_code-lceclassifier_seq_strc_bench.py --seq_model_name self_seq --feature_type seq_strc --reduced_dim_strc 28 --reduced_dim_seq 255 --result_dir_suffix _change_strc_embed`. Expected runing time: within 1 min Nvidia A100.
