...
class EnzymeCommissionZyx(data.ProteinDataset):
    ...
    def __init__(self, path, test_cutoff=0.95, verbose=1, **kwargs):
        ...
        pkl_file = os.path.join(path, "EnzymeCommission", self.processed_file)

        self.pdb_files = []
        for split in ["test","valid","train"]:
            # split_path = utils.extract(os.path.join(path, "%s.zip" % split))
            self.pdb_files += sorted(glob.glob(os.path.join(path, "EnzymeCommission", split, "*.pdb")))
        if os.path.exists(pkl_file):
            self.load_pickle(pkl_file, verbose=verbose, **kwargs)
        else:
            self.load_pdbs(self.pdb_files, verbose=verbose, **kwargs)
            self.save_pickle(pkl_file, verbose=verbose)
        #if len(self.pdb_files) > 0:
            #self.filter_pdb()

        tsv_file = os.path.join(path, "EnzymeCommission", "nrPDB-EC_annot.tsv")
    ...
    def filter_pdb(self):
        types = ['Bacteria']
        validList = []
        sheet = pd.read_excel("../owndata/1000 antigen.xlsx", "pos")
        for row in sheet.index.values:
            type = sheet.iloc[row, 8]
            if type in types and sheet.iloc[row, 0] not in validList:
                validList.append(sheet.iloc[row, 0])
        sheet = pd.read_excel("../owndata/1000 antigen.xlsx", "neg")
        for row in sheet.index.values:
            type = sheet.iloc[row, 8]
            if type in types and sheet.iloc[row, 0] not in validList:
                validList.append(sheet.iloc[row, 0])
        newarr=[]
        for item in self.pdb_files:
            if item.split('-')[1] not in validList:
                self.pdb_files.remove(item)
            else:
                newarr.append(item)
        self.pdb_files = newarr
    ...
    def load_annotation(self, tsv_file, pdb_ids):
        ...
        for pdb_id in pdb_ids:
            self.pos_targets.append(pos_target)
